...
router.post('/emergency-alert', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await whatsAppService.sendEmergencyAlert(whatsappContacts, alertData as { threatLevel: string; transcript: string; reasoning: string; confidence: number; location?: { latitude: number; longitude: number }; audioUrl?: string });
...
router.post('/contact-form', async (req, res: Response) => {
  try {
    ...
    const result = await whatsAppService.sendContactFormMessage(recipientNumber, formData as { name: string; email: string; message: string; phone?: string });
